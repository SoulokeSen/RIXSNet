...
class EGNN_C_Block(nn.Module):
    ...
    def forward(self, s, v, edge_index):
        ...
        print("salgebra.flatten(pos_message)", self.algebra.flatten(pos_message).shape)
        ...
        print("pos_message_aggr", pos_message_aggr.shape)
...
#        batch = batch.to("cuda")
        ...
        print("shape of loc in batch", batch.loc.shape)
        ...
        print("size of pred_pos", pred_pos.shape)
        pred_pos = pred_pos[:, 0, 1:4]
        pred_pos = pred_pos.reshape(batch_size, -1, 3) + coords
        pred_pos = pred_pos.reshape(-1,3)
        print("pred_pos after reshape, batch_y", pred_pos.shape, batch.y.shape)
        loss = F.mse_loss(pred_pos, batch.y.reshape(-1, 3), reduction="none").mean(dim=1) #do you calculate then here node-level outputs
