...
class Trainer:
    ...
    def train_step(
        ...
        loss, outputs, loss_comp, _ = model(batch, self.train_state["global_step"], prefix, loss)
        ...
        if self.train_state["global_step"] % self.print_interval == 0:
            print(
                f"Step: {self.train_state['global_step']} (Training) Loss: {loss:.4f}", [f"{x:.8f}" for x in loss_comp]
            )
    ...
    def test_loop(
        ...
        loss_components = torch.zeros(2, dtype=torch.float64)
        num_sample = 0
        for batch_idx, batch in enumerate(test_loader):
            ...
            _, outputs, loss_comp, _ = model(batch, batch_idx, prefix, loss)
...
#        num_batch_val = len(test_loader)
        ...
        for batch_idx, batch in enumerate(debug_loader):
            ...
            batch = to_device(batch, self.device)
            batch_size = batch.ptr.shape[0] - 1
            _, outputs, loss_comp, sp = model(batch, batch_idx, "debug", loss)
            spectra.append(sp)
        ...
        loss_components /= num_sample
        plot.dumpandplot(loss_components,spectra,batch_outputs, datastr, self.run_dir, self.logtransform, self.pca)
# =============================================================================
